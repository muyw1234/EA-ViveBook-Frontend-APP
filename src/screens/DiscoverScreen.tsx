    ...
    return (
      <View key={u._id} style={styles.itemRow}>
        <View style={styles.itemLeft}>
          <Avatar.Text
            size={40}
            label={(u.name || 'U').substring(0, 2).toUpperCase()}
    ...
    return (
      <View key={typeof a === 'string' ? a : a._id || authorName} style={styles.itemRow}>
        <View style={styles.itemLeft}>
          <Avatar.Text
            size={40}
            label={(authorName || 'A').substring(0, 2).toUpperCase()}
            style={{ backgroundColor: '#D183BA' }}
          />
          <Text variant="titleMedium" style={{ marginLeft: 10 }}>
            {authorName}
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        ...
        <View style={styles.section}>
          ...
          {authors.length > 0 ? (
            authors.map(renderAuthorItem)
          ) : (
            <Text style={styles.emptyText}>No hay autores disponibles.</Text>
          )}
        </View>
