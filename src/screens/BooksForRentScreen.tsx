...
export default function BooksForRentScreen() {
      ...
      const fetchBooksAndReservations = async () => {
        try {
          const response = await api.get(
            `/libros/type/ALQUILER?page=${page}&limit=${ITEMS_PER_PAGE}`,
          );
  ...
  const handleRentDirectly = async (book: any) => {
    ...
    try {
      ...
      const response = await api.get(`/libros/type/ALQUILER?page=${page}&limit=${ITEMS_PER_PAGE}`);
      const resData = response.data?.data;
      const booksArray = Array.isArray(resData?.data)
        ? resData.data
        : Array.isArray(resData)
      ...
      showAlert(t('error'), t('rent_err') || 'No se pudo completar el alquiler');
    }
  };
        ...
        <View style={styles.cardButtons}>
          <Menu
            ...
            anchor={
              ...
              >
                {book.isReserved ? t('reserved', 'Reservado') : t('rent_action')}
              </Button>
            }
